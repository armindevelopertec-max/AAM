...
export default function ClientManager({ initialClients }: { initialClients: Client[] }) {
  ...
  async function handleSave(id: number) {
    setError(null);
    try {
      await updateClient(id, { name: editName, email: editEmail, phone: editPhone });
      setEditingId(null);
      await refresh();
    } catch (err) {
      setError(err instanceof Error ? err.message : "Error desconocido");
      ...
      <section className="rounded-lg border border-neutral-200 bg-white p-6 dark:border-neutral-800 dark:bg-neutral-900">
        ...
        <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-1 sm:col-span-2">
            ...
            <input id="phone" type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} required className={inputClass} />
          </div>
          <button
            type="submit"
